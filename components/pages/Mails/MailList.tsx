...
const MailList: React.FC = () => {
  ...
  const getUploads = async () => {
    try {
      const response = await lighthouse.getUploads(
        "2db3bf8f.1dd34afe207d45af9e2e1d6f0e1e2459"
      );
  ...
  const fetchFileDetails = async (cid: string) => {
    try {
      ...
      if (toMatch && subjectMatch && bodyMatch) {
        ...
        // Resolve ENS name for the "to" address
        resolveEnsName(toMatch[1]);
      }
    } catch (error) {
  ...
  // Pagination logic
  const filteredFiles = files.filter(file => fileDetails[file.cid]?.to && fileDetails[file.cid]?.subject && fileDetails[file.cid]?.body);
          ...
          fileDetails[file.cid]?.to && fileDetails[file.cid]?.subject && fileDetails[file.cid]?.body && (
            ...
            >
              <div className="grid grid-cols-3 gap-4">
                <p className="truncate">To: {ensNames[fileDetails[file.cid]?.to] || fileDetails[file.cid]?.to}</p>
      ...
      {selectedFile && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60">
          ...
          >
            ...
            <p className="text-lg mb-2"><strong>To:</strong> {ensNames[selectedFile.to] || selectedFile.to}</p>
